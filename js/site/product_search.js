define(['jquery','./common','handlebar'],function(jquery, common, Handlebars){
	...
	function ready(){
		$.ajax({
			...
			success:function(rs){
				...
				//设置默认选中配件类型分类
				setPartsType(productTyprId,partsTypeId);
				//产品分类设置点击事件
	...
	function findProducts(productTypeId,partsTypeId,conditon,pageNum,pageSize){
		...
		$.ajax({
			...
			crossDomain:true,
			type:"post",
			data:{'productTypeId':productTypeId,'partsTypeId':partsTypeId,
				   'name':conditon,'pageNum':pageNum,'pageSize':pageSize},
			success:function(rs){
				//设定图片地址
				...
				//获取数据插入页面
				var tpl=$("#product_list_tpl").html();
				var func=Handlebars.compile(tpl);
				$(".probox").html(func(rs.data.data));
				//处理分页显示
				$(".btn_prev").attr("data-page",rs.data.prePage);   //赋值
				$(".btn_next").attr("data-page",rs.data.nextPage);
				$(".page_num").attr("data-page-num",rs.data.pageNum);
				$(".page_count").html("共"+rs.data.totalPage+"页");    //将值添加上并显示
				//绑定点击事件
	...
	function setPartsType(productTypeId,partsTypeId){
		...
		//设置选中时 样式改变
		$("#parts_type_container").find("a").each(function(index,obj){
			$(obj).removeClass("selected");
			if($(obj).attr("patrs-type-id")==partsTypeId){
				$(obj).addClass("selected");
			}
		});
		//清除最后一个分割线
	    ...
	    var partsTid=$(this).attr("patrs-type-id");
