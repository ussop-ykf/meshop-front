		...
		$.ajax({
			...
			success:function(rs){
				...
				$.each(rs.data, function(index, item) {  
					// 创建模板字符串
					 console.log(item);
					var template =  '<li class="unew-li"><a target="_blank" href="product_detail.html?id='+item.id+'&img='+item.iconUrl+'&price='+item.price+'&name='+item.name+'&detail='+item.detail+'" class="unewli-box"><div class="unewli-img"><img src="images/products/'+item.iconUrl+'" data-original="images/products/'+item.iconUrl+'" /></div>  <h2 class="font-ublack font-title">'+item.name+'</h2><p class="price-box"></p><p class="font3 font-size2" style="padding-top:0px;"><span>'+item.price+'</span><span style="margin-left:50px;">市场价</span><span class=" text-de">'+item+100+'</span></p></a></li>';  
					// 将模板字符串插入到页面中  
